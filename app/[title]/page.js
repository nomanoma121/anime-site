...
export default async function AnimeDetail({ params }) {
  ...
  const schedule = anime?.startDate.year + "年" + anime?.startDate.month + "月" + anime?.startDate.day + "日 ～ ";
  const studio = anime?.studios.nodes?.map((e) => e.name + ", ");
  const mainCharacters = anime?.characters.nodes?.slice(0, 5);
  const averageScore = anime?.averageScore !== undefined ? anime?.averageScore : 0;
  const rating = averageScore / 20;

  return (
    <div className={styles.container}>
      <div className={styles.imageContainer}>
        <img src={anime?.coverImage.extraLarge} className={styles.animeImage}/>
      </div>
      <div className={styles.detailContainer}>
        ...
        <div className={styles.genres}>
          {anime?.genres.map((e) => (
            <div key={e} className={styles.genre}>{e}</div>
          ))}
        </div>
        ...
        <div className={styles.characters}>
          {mainCharacters?.map((e, index) => (
            <div className={styles.characterDetail} key={`container${index}`}>
              <img src={e.image.medium} key={index} className={styles.characterImage}/>
              <h3 key={e.name.native} className={styles.characterName}>{e.name.native}</h3>
            </div>
          ))}
        </div>
        <div>
